...
<div class="row row_user_dashboard">
    ...
    <h3 class="entry-title">Pagos por Vistas de Propiedades</h3>       
    ...
    <?php 
        ...
        $buyer_invoices = get_posts($args); 
        
    ?>

    <div class="col-md-12" style='    padding-left: 60px;'>
        <table style='width: 100%;'>
            <thead style='padding: 10px 0px; margin: 15px 0px; background: #e7e7e7; color: #263238;'>
                <tr>
                    <th style="font-weight: bold;font-size: 14px; padding: 10px 20px;color: #263238;">Membresía</th>
                    <th style="font-weight: bold;font-size: 14px; padding: 10px 20px;color: #263238;">Fecha</th>
                    <th style="font-weight: bold;font-size: 14px; padding: 10px 20px;color: #263238;">Precio</th>
                    <th style="font-weight: bold;font-size: 14px; padding: 10px 20px;color: #263238;">Medio de Pago</th>
                    <th style="font-weight: bold;font-size: 14px; padding: 10px 20px;color: #263238;">Propiedad</th>
                </tr>
            </thead>
            <tbody>
                <?php if($buyer_invoices && count($buyer_invoices) ):?>
                    <?php foreach( $buyer_invoices as $buyer_invoice):?>
                    
                    <?php 
                    
                    $subscription_id = $buyer_invoice->ID;

                    $buyer_membership_type = get_post_meta($subscription_id, 'buyer_membership_type', true);
                    $buyer_payment_method = get_post_meta($subscription_id, 'buyer_payment_method', true);

                    $buyer_membership = 'seller_membership_'.$buyer_membership_type;

                    $buyer_membership_name = get_option($buyer_membership.'_name', '');
                    $buyer_membership_price = get_option($buyer_membership.'_price', '');
                    ...
                    //$buyer_membership_state = 'Activo'

                    $property_url = 'N/A';

                    if($buyer_membership_type == 'single'){
                       // days_left($pretty_date, $buyer_membership_type);
                        
                       $property_id = get_post_meta($subscription_id, 'buyer_membership_property', true);

                       $property_title = get_the_title(intval($property_id));

                       $property_url = '<a title="'.$property_title.'" href="'.get_the_permalink(intval($property_id)).'">Ver Propiedad</a>';
                    ...
                    <tr>
                        <td><?php echo $buyer_membership_name;?></td>
                        <td><?php echo $pretty_date;?></td>
                        <td><?php echo $buyer_membership_price; ?></td>
                        <td><?php echo $buyer_payment_method; ?></td>
                        <td><?php echo $property_url; ?></td>
                    </tr>
                <?php endforeach;?>
                <?php else:?>
                <tr>
                    <td colspan="5">No hay pagos realizados.</td>
                </tr>
                <?php endif;?>
